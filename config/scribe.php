...
return [
    ...
    'title' => 'Komiut / 2Safiri Matatu API',

    // A short description of your API. Will be included in the docs webpage, Postman collection and OpenAPI spec.
    'description' => 'REST API for the Komiut / 2Safiri matatu SACCO platform: authentication, vehicles, routes, queues, bookings, transactions, saccos, users and loyalty points.',

    // Text to place in the "Introduction" section, right after the `description`. Markdown and HTML are supported.
    'intro_text' => <<<'INTRO'
            This documentation covers the Komiut / 2Safiri Matatu API.

            <aside>All endpoints require the <code>X-App-Key</code> brand header, and authenticated endpoints require a Sanctum bearer token.
    ...
    'routes' => [
        [
            'match' => [
                // Document the versioned consumer API only — not the legacy
                // unversioned aliases or the bank/Daraja webhook callbacks.
                'prefixes' => ['api/v1/*'],
    ...
    'auth' => [
        ...
        'extra_info' => 'Obtain a bearer token from the <code>POST api/login</code> or <code>POST api/register</code> endpoints. All requests must also send the <code>X-App-Key</code> brand header.',
    ],
    ...
    // Left EMPTY on purpose: docs are generated in an environment with no live DB
    // (default connection is mysql), and response calls are disabled, so there is
    // nothing to wrap in a transaction. This avoids CouldntStartDatabaseTransaction.
