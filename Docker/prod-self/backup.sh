...
#           docker compose -f docker-compose.prod-self.yml exec -T db psql -U komiut -d komiut
set -euo pipefail
...
docker compose -f docker-compose.prod-self.yml exec -T db pg_dump -U komiut -d komiut | gzip > "$FILE"
