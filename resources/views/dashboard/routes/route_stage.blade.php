...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h5><i class='fas fa-map-marker-alt'></i> Route Stage</h5>
                </div><!-- /.col -->
                <div class="col-sm-6">
                        ...
                        <div class="card">
                            <div class="card-header">
                                <div class='row d-flex align-items-center'>
                                    <div class='col-sm-8'>
                                        <h5 class="m-0"><b>{{ $routeStage->place->name }}</b> (
                                            {{ $routeStage->route->from->name }}-{{ $routeStage->route->to->name }})</h5>
                                    </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel"><i class='fas fa-plus'></i> <span>Update</span> Stage
                        Location</h5>
                ...
                <div class="modal-body" style="border-bottom: 1px solid rgb(200,200,200);">
                    ...
                    <form action="{{ url('routes/stage/update') }}" method="POST" class="row">
                        ...
                        <div class="col-sm-12 form-group">
                            ...
                            <input type='text' name="place" class='form-control' placeholder='Place Name'
                                value="{{ $routeStage->place->name }}" readonly>
                        </div>
                        <div class="col-sm-6 form-group">
                            <label>Latitude</label>
                            <input id='latitude' name="latitude" class='form-control' placeholder='Latitude'
                                value="{{ $routeStage->latitude != null ? $routeStage->latitude : $routeStage->route->from->latitude }}"
                                readonly />
                        </div>
                        <div class="col-sm-6 form-group">
                            <label>Longitude</label>
                            <input id='longitude' name="longitude" class='form-control' placeholder="Longitude"
                                value="{{ $routeStage->longitude != null ? $routeStage->longitude : $routeStage->route->from->longitude }}"
                                readonly>
                        </div>
                        <div class="col-sm-12 form-group">
                            <label>Status</label>
...
@push('js')
    <!--   Optional JS   -->
    <script
        src="https://maps.googleapis.com/maps/api/js?key=AIzaSyAFrY5fH-gBUGMk6zfFnmk7aHZp-Dzzdzo&libraries=places&region=KE">
    </script>
    ...
    <script>
        $(document).ready(function() {
            ...
            function initMap() {
                const myLatlng = {
                    lat: parseFloat(
                        "{{ $routeStage->latitude != null ? $routeStage->latitude : $routeStage->route->from->latitude }}"
                        ),
                    lng: parseFloat(
                        "{{ $routeStage->longitude != null ? $routeStage->longitude : $routeStage->route->from->longitude }}"
                        )
                };
