...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0"><i class='fas fa-cog'></i> Gender <b>Settings</b></h1>
                </div><!-- /.col -->
                <div class="col-sm-6 text-right">
                    @can('Add Gender Settings')
                        <button class="btn btn-primary btn-sm btn-launch-modal" data-toggle="modal" data-target="#userModal"><i
                                class='fas fa-plus'></i> Add Gender</button>
                    @else
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="{{ url('home') }}">Home</a></li>
                            <li class="breadcrumb-item active">Gender Settings</li>
                        </ol>
                    @endcan
                </div><!-- /.col -->
            </div><!-- /.row -->
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel"><i class='fas fa-plus'></i> <span>New </span> Gender
                    </h5>
                    <button type="button" class="btn-close" data-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form method="POST" action="{{ url('settings/gender/add') }}" class="row">
                        @csrf
                        <input type='hidden' name='id' value='0'>
                        <div class='col-sm-12 form-group'>
                            <label>Name</label>
                            <input type='text' placeholder="Gender Name" name="name" class='form-control' autofocus
                                required />
                        </div>
                        <div class='col-sm-12 form-group'>
                            <label>Status</label>
    ...
    <script>
        $(document).ready(function() {
            var table = $('.table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ url('settings/datatable/gender') }}",
                    data: function(d) {
