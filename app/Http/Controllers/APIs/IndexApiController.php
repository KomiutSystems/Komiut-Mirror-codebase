...
class IndexApiController extends Controller
{
    ...
    public function copyRedeemedPoints(Request $request)
    {
        ...
        //return $created_at;
        $json = json_decode(file_get_contents($url), true);
        foreach ($json["redeemed_points"] as $redeemed_point) {
            //"mpesa_booking_callback_id", "mpesa_qrcode_payment_id", "points", "trans_date"
            $point = null;
            ...
            if ($vehicle != null && $point != null) {
                if (
                    ...
                    DB::table('redeemed_points')->insert([
                        "point_id" => $point->id,
                        "vehicle_id" => $vehicle->id,
                        "redeemed_points" => $redeemed_point['redeemed_points'],
                        "status" => $redeemed_point['status'],
                        'created_at' => Carbon::parse($redeemed_point['created_at']),
                        'updated_at' => Carbon::parse($redeemed_point['updated_at']),
                    ]);
                }
            }
        }
        return response()->json(['success' => "Redeemed Points imported successfully"]);
    }
