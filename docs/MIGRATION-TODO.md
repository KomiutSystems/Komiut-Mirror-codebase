...
- **18 Sep, morning.** Phase 2 done the same night on Tony's go: `komiut.com`+`www` 04:10, `2safiri.co.ke`+`www` 04:12; the apexes serve the dashboards (nginx proxies non-API paths to App Runner); `legacy.komiut.com` / `legacy.2safiri.co.ke` → Mumbai read-only. First morning clean: 0 unattributed on Safaricom, NCBA and Co-op. Fixed live: driver app's `crew_id=null` (500). 25 NCBA aggregator payments that reached legacy between the flips imported through the recorder (KES 12,341); reconciliation legacy 0 / deficit 0. SES production access was DENIED (case 178969270900625): mail now goes out over the `info@komiut.com` Private Email SMTP legacy used, and `forgot-password` no longer 500s on a dead mailer. The TenaSMS key is dead on both systems ("Invalid credentials"): driver SMS sign-in passwords and mobile password resets do not send until a working SMS account is configured.
